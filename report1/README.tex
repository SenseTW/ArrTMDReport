...
\end{enumerate}
\section{研究方法 }
\label{sec:org74b7360}
\subsection{研究架構}
\label{sec:org7ad3b7c}
\begin{figure}[htbp]
\centering
...
\label{sec:orgce12b41}
\begin{enumerate}
\item 文獻檢閱
\label{sec:orgfafe76b}
...
\bottomrule
\end{tabular}
\end{table}\footnotetext[4]{\label{org8895a3e}MIT 授權允許被授權人「有權利使用、複製、修改、合併、出版發行、散布、再授權和/或販售軟體及軟體的副本，及授予被供應人同等權利，惟服從以下義務：『在軟體和軟體的所有副本中都必須包含以上著作權聲明和本許可聲明。』」，中文說明 2018 年 12 月 22 日摘自維基百科\url{https://zh.wikipedia.org/wiki/MIT\%E8\%A8\%B1\%E5\%8F\%AF\%E8\%AD\%89}，英文 MIT 協定全文請見：\url{https://opensource.org/licenses/MIT}。}\footnotetext[5]{\label{org0adf0ea}CC BY-SA（姓名標示-相同方式分享） 3.0 台灣之授權條款為依據「創用 CC 公眾授權條款」中授權姓名標示─相同方式分享。本授權條款允許使用者重製、散布、傳輸以及修改著作（包括商業性利用）。若使用者修改該著作時，僅得依本授權條款或與本授權條款類似者來散布該衍生作品。使用時必須按照著作人指定的方式表彰其姓名。詳細授權條款請見：\url{https://creativecommons.org/licenses/by-sa/3.0/tw/legalcode}}\footnotetext[6]{\label{orga77e576}CC BY-SA 4.0 (Attribution-Sharealike) International，詳細英文授權條款請見：\url{https://creativecommons.org/licenses/by-sa/4.0/}。}
\end{enumerate}

\section{文獻檢閱 }
\label{sec:org581eab8}

本章回顧「網路社群與資訊科技社群」、「科技治理體系及溝通缺口」、「線上公民與政治參與的要素」以及「數位工具的幫助與限制」作為研究背景。
\subsection{網路社群與資訊科技社群}
\label{sec:org7dd6f2c}
...
\label{sec:orgf0f6480}
\subsection{設計敏捷方法問題定義與原型開發}
\label{sec:orge4dd8af}
...
\item 參與途徑說明
\label{sec:orgf4f027f}

本節說明 圖 \ref{fig:service-blueprint} 各階段細節。

\begin{enumerate}
\item 前期議題設定：
\label{sec:org20bc98d}
...
\item 數位原民參與手冊供網路社群理解政策形成機制與公民參與方法\footnote{主要與現為軟體工程師的樹黨發起人討論。 樹黨發起人之目的為倡議 Public Money Public Code，撰有〈二十年後的政府軟體〉一文 \url{http://treesparty.tw/2018/06/17/public-money-public-code/}。\label{org075905a}}
\end{enumerate}

\section{線上線下整合的網路參與機制設計 }
\label{sec:org1de3723}
...
\label{sec:org98d2207}
\begin{enumerate}
\item 應用場景
\label{sec:org45073d9}
...
\subsection{線上線下串接案例 }
\label{sec:org82d8ff7}
前述分別介紹線上意見整理工具 sense.tw 與線下的議題小聚，此節以案例介紹如何串接線上數位工具與線下討論，加深網路社群討論科技政策之品質。
\begin{enumerate}
\item Public Money, Public Code
\label{sec:org3055029}

Public Money, Public Code 為國際開源社群長期推動由政府資助開發的軟體應開放原始碼的運動\footnote{Public Money, Public Code 活動由歐洲自由軟體基金會發起，要求制定法律，明訂以公務機關的經費，為公務機關開發的軟體，必須以自由開源軟體授權釋出，截至 2018/12/21 為止全球已有 158 個組織與 19130 個人已經連署公開信表達支持。詳見 \url{https://publiccode.eu/zh-tw/}。}。在開源人年會（COSCUP）中大高雄 Linux 協會（KaLUG）與樹黨\textsuperscript{\ref{org075905a}}開始討論，於是舉辦之議題工作坊以此為主題進行討論。
...
\section{附錄}
\label{sec:org8b103d0}
\subsection{訪談對象與訪談形式}
\label{sec:org74861ba}
{\small \rowcolors[]{2}{contiYellow!5}{contiYellow!20}
...
\end{itemize}
\item 訪談問題
\label{sec:org3ef3770}
\begin{enumerate}
\item 怎樣蒐集社群意見、倡議產生政策影響力
\label{sec:orgf1104d1}
簡述政策參與（促進）的經驗
\begin{enumerate}
\item 政府部門參與過的政策形式，什麼樣的方式參與？與政府開會的角色是什麼？
\label{sec:org99a2a51}
\item 用什麼架構分析政策形成、公民參與？
\label{sec:org1993a91}
\end{enumerate}
\item 政策參與（促進）的方式
\label{sec:org522f780}
\begin{enumerate}
\item 怎麼倡議？
\label{sec:orgde6540a}
\item 怎麼組織？ 會舉辦實體會議嗎？）
\label{sec:org91b73ad}
\item 現行體制（政府意見陳達、溝通」）問題點
\label{sec:org5fb7860}
\item 案例建議與原因？
\label{sec:org5cbc912}
\item 任何範本、方法？
\label{sec:org2f27b73}
\end{enumerate}
\item 促進社群討論的方式
\label{sec:org5a46a54}
\item 如何保持內部成員資訊即時更新、通知同步
\label{sec:org9edd2b8}
\item 如何鼓勵/促進內部人員討論（討論風氣、鼓勵發言的文化）
\label{sec:orgefd0e85}
\item 使用的討論工具
\label{sec:org347492f}
\item 使用什麼工具協助討論？
\label{sec:orgb648108}
\item 使用什麼平台作團隊內知識分享、討論？
\label{sec:orga8a50ca}
\item 平台的優缺點?
\label{sec:org19f06f8}
\item 哪些是覺得必要的功能？
\label{sec:org2a81dba}
\end{enumerate}
\end{enumerate}


\subsection{議題釐清工具相關開發資訊}
\label{sec:orga054375}
\begin{enumerate}
\item 源碼庫
\label{sec:org1497cc8}
\begin{enumerate}
...
\end{enumerate}
\item 部落格
\label{sec:org77ec6f4}
\url{https://medium.com/sense-tw}
\end{enumerate}
\subsection{民眾語彙腳本}
\label{sec:org73bea0b}
\begin{enumerate}
\item 對公部門介紹何為網路社群應如何比喻
\label{sec:org04b80aa}
