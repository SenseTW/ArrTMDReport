...
\bottomrule
\end{tabular}
\end{table}\footnotetext[4]{\label{org9ca4f7c}MIT 授權允許被授權人「有權利使用、複製、修改、合併、出版發行、散布、再授權和/或販售軟體及軟體的副本，及授予被供應人同等權利，惟服從以下義務：『在軟體和軟體的所有副本中都必須包含以上著作權聲明和本許可聲明。』」，中文說明 2018 年 12 月 22 日摘自維基百科\url{https://zh.wikipedia.org/wiki/MIT\%E8\%A8\%B1\%E5\%8F\%AF\%E8\%AD\%89}，英文 MIT 協定全文請見：\url{https://opensource.org/licenses/MIT}。}\footnotetext[5]{\label{orga940ae0}CC BY-SA（姓名標示-相同方式分享） 3.0 台灣之授權條款為依據「創用 CC 公眾授權條款」中授權姓名標示─相同方式分享。本授權條款允許使用者重製、散布、傳輸以及修改著作（包括商業性利用）。若使用者修改該著作時，僅得依本授權條款或與本授權條款類似者來散布該衍生作品。使用時必須按照著作人指定的方式表彰其姓名。詳細授權條款請見：\url{https://creativecommons.org/licenses/by-sa/3.0/tw/legalcode}}\footnotetext[6]{\label{org04b7f9c}CC BY-SA 4.0 (Attribution-Sharealike) International，詳細英文授權條款請見：\url{https://creativecommons.org/licenses/by-sa/4.0/}。}
\end{enumerate}

\section{文獻檢閱 }
\label{sec:orgb2e8536}
...
\end{itemize}
\item 訪談問題
\label{sec:orgad4adfc}
\begin{enumerate}
\item 怎樣蒐集社群意見、倡議產生政策影響力
\label{sec:org34fbb39}
簡述政策參與（促進）的經驗
\begin{enumerate}
\item 政府部門參與過的政策形式，什麼樣的方式參與？與政府開會的角色是什麼？
\label{sec:org4e21e15}
\item 用什麼架構分析政策形成、公民參與？
\label{sec:org4758180}
\end{enumerate}
\item 政策參與（促進）的方式
\label{sec:org61b490e}
\begin{enumerate}
\item 怎麼倡議？
\label{sec:org540e1c5}
\item 怎麼組織？ 會舉辦實體會議嗎？）
\label{sec:org708d3bf}
\item 現行體制（政府意見陳達、溝通」）問題點
\label{sec:orge1b0927}
\item 案例建議與原因？
\label{sec:org2781ab6}
\item 任何範本、方法？
\label{sec:org9c7a59e}
\end{enumerate}
\item 促進社群討論的方式
\label{sec:orgeace2f0}
\item 如何保持內部成員資訊即時更新、通知同步
\label{sec:org65f53fb}
\item 如何鼓勵/促進內部人員討論（討論風氣、鼓勵發言的文化）
\label{sec:orgd4dfc3c}
\item 使用的討論工具
\label{sec:org52e4b27}
\item 使用什麼工具協助討論？
\label{sec:org673dcd9}
\item 使用什麼平台作團隊內知識分享、討論？
\label{sec:org2662027}
\item 平台的優缺點?
\label{sec:orgbd699ef}
\item 哪些是覺得必要的功能？
\label{sec:orgec1ea2a}
\end{enumerate}
\end{enumerate}


\subsection{議題釐清工具相關開發資訊}
\label{sec:orgca1f1b4}
\begin{enumerate}
\item 源碼庫
\label{sec:org305ad5d}
\begin{enumerate}
...
\label{sec:orgd09a366}
每個層級的長官關心的議題大小不同，以政務委員為例，頂多看到第二級。

\begin{center}
\label{tab:orgee3454c}
\rowcolors[]{2}{contiYellow!5}{contiYellow!20}
\begin{tabular}{lll}
\toprule
層次 & 關心議題的動機 & 長官\\
