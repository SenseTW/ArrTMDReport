...
\large 如何促進科技社群網路參與科技政策討論建議報告 - 以資訊科技社群為例 (草稿)}
\hypersetup{
...
\end{enumerate}
\section{研究方法 }
\label{sec:orgeca620c}
\subsection{研究架構}
\label{sec:org06cffbd}
\begin{figure}[htbp]
\centering
...
\label{sec:orge5f1b83}
\begin{enumerate}
\item 文獻檢閱
\label{sec:org58edd87}
...
「實踐社群」這樣崇尚實作的文化，使得一個人在這樣的社群的影響力是建立在他為社群貢獻過什麼事蹟，因此網路的暱稱 ID  比本名還真，基本上可以算在江湖 (community) 的藝名 (nickname)，任何職銜在此也不重要，大家認可的是該人做過的貢獻，而不是他是否為理事長、發起人。社群所形成的文化，也就是所謂的默契，會讓社群的意見領袖，受到一定程度的規範，若是意見領袖打破這個默契，就會在所屬的社群中影響力下降。這樣的治理模型在自由開源社群特別常見：「仁慈的獨裁者」（benevolent dictator）必須保持仁慈，否則巨大的分歧會導致專案被復刻（fork）並由新的領導所掌管。
...
\label{sec:orgd5d9ebf}
\subsection{設計敏捷方法問題定義與原型開發}
\label{sec:org10780ff}
...
\item 參與途徑說明
\label{sec:org3499944}

本節說明 圖 \ref{fig:service-blueprint} 各階段細節。

\begin{enumerate}
\item 前期議題設定：
\label{sec:org40ab1e1}
...
\item 數位原民參與手冊供網路社群理解政策形成機制與公民參與方法\footnote{主要與現為軟體工程師的樹黨發起人討論。 樹黨發起人之目的為倡議 Public Money Public Code，撰有〈二十年後的政府軟體〉一文 \url{http://treesparty.tw/2018/06/17/public-money-public-code/}。\label{org4976b12}}
\end{enumerate}

\section{線上線下整合的網路參與機制設計 }
\label{sec:org65f1799}
...
\label{sec:orgebdfdba}
\begin{enumerate}
\item 應用場景
\label{sec:org0f09385}
...
\subsection{線上線下串接案例 }
\label{sec:org57c63f7}
前述分別介紹線上意見整理工具 sense.tw 與線下的議題小聚，此節以案例介紹如何串接線上數位工具與線下討論，加深網路社群討論科技政策之品質。
\begin{enumerate}
\item Public Money, Public Code
\label{sec:org4491a6f}

Public Money, Public Code 為國際開源社群長期推動由政府資助開發的軟體應開放原始碼的運動\footnote{Public Money, Public Code 活動由歐洲自由軟體基金會發起，要求制定法律，明訂以公務機關的經費，為公務機關開發的軟體，必須以自由開源軟體授權釋出，截至 2018/12/21 為止全球已有 158 個組織與 19130 個人已經連署公開信表達支持。詳見 \url{https://publiccode.eu/zh-tw/}。}。在開源人年會（COSCUP）中大高雄 Linux 協會（KaLUG）與樹黨\textsuperscript{\ref{org4976b12}}開始討論，於是舉辦之議題工作坊以此為主題進行討論。
...
\section{附錄}
\label{sec:org2299967}
\subsection{訪談對象與訪談形式}
\label{sec:org4571f48}
{\small \rowcolors[]{2}{contiYellow!5}{contiYellow!20}
...
\end{itemize}
\item 訪談問題
\label{sec:orgda887f1}
\begin{enumerate}
\item 怎樣蒐集社群意見、倡議產生政策影響力
\label{sec:orgdfc4358}
簡述政策參與（促進）的經驗
\begin{enumerate}
\item 政府部門參與過的政策形式，什麼樣的方式參與？與政府開會的角色是什麼？
\label{sec:org8fab14d}
\item 用什麼架構分析政策形成、公民參與？
\label{sec:orged31b6f}
\end{enumerate}
\item 政策參與（促進）的方式
\label{sec:org96226c0}
\begin{enumerate}
\item 怎麼倡議？
\label{sec:orgd925cc1}
\item 怎麼組織？ 會舉辦實體會議嗎？）
\label{sec:org0d5fc3e}
\item 現行體制（政府意見陳達、溝通」）問題點
\label{sec:orgdfbc8d1}
\item 案例建議與原因？
\label{sec:orgf5b8807}
\item 任何範本、方法？
\label{sec:org489d2f4}
\end{enumerate}
\item 促進社群討論的方式
\label{sec:org7074067}
\item 如何保持內部成員資訊即時更新、通知同步
\label{sec:org8caff0b}
\item 如何鼓勵/促進內部人員討論（討論風氣、鼓勵發言的文化）
\label{sec:org1a9009a}
\item 使用的討論工具
\label{sec:org2828a35}
\item 使用什麼工具協助討論？
\label{sec:orgd986aae}
\item 使用什麼平台作團隊內知識分享、討論？
\label{sec:org9b84949}
\item 平台的優缺點?
\label{sec:org23bd2cf}
\item 哪些是覺得必要的功能？
\label{sec:org0982739}
\end{enumerate}
\end{enumerate}


\subsection{議題釐清工具相關開發資訊}
\label{sec:orgcef7d04}
\begin{enumerate}
\item 源碼庫
\label{sec:org231c788}
\begin{enumerate}
...
\end{enumerate}
\item 部落格
\label{sec:org8ed9f75}
\url{https://medium.com/sense-tw}
\end{enumerate}
\subsection{民眾語彙腳本}
\label{sec:orgf7f6a66}
\begin{enumerate}
\item 對公部門介紹何為網路社群應如何比喻
\label{sec:org44deda4}
