...
\end{enumerate}
\section{研究方法 }
\label{sec:org99ec7a9}
\subsection{研究架構}
\label{sec:org4f7f128}
\begin{figure}[htbp]
\centering
...
\label{sec:orgb2bb4f4}
\begin{enumerate}
\item 文獻檢閱
\label{sec:orgb344279}
...
\bottomrule
\end{tabular}
\end{table}\footnotetext[4]{\label{orgf5c36d2}MIT 授權允許被授權人「有權利使用、複製、修改、合併、出版發行、散布、再授權和/或販售軟體及軟體的副本，及授予被供應人同等權利，惟服從以下義務：『在軟體和軟體的所有副本中都必須包含以上著作權聲明和本許可聲明。』」，中文說明 2018 年 12 月 22 日摘自維基百科\url{https://zh.wikipedia.org/wiki/MIT\%E8\%A8\%B1\%E5\%8F\%AF\%E8\%AD\%89}，英文 MIT 協定全文請見：\url{https://opensource.org/licenses/MIT}。}\footnotetext[5]{\label{org2ba89c5}CC BY-SA（姓名標示-相同方式分享） 3.0 台灣之授權條款為依據「創用 CC 公眾授權條款」中授權姓名標示─相同方式分享。本授權條款允許使用者重製、散布、傳輸以及修改著作（包括商業性利用）。若使用者修改該著作時，僅得依本授權條款或與本授權條款類似者來散布該衍生作品。使用時必須按照著作人指定的方式表彰其姓名。詳細授權條款請見：\url{https://creativecommons.org/licenses/by-sa/3.0/tw/legalcode}}\footnotetext[6]{\label{org2b216d4}CC BY-SA 4.0 (Attribution-Sharealike) International，詳細英文授權條款請見：\url{https://creativecommons.org/licenses/by-sa/4.0/}。}
\end{enumerate}

\section{文獻檢閱 }
\label{sec:org7895b1f}

本章回顧「網路社群與資訊科技社群」、「科技治理體系及溝通缺口」、「線上公民與政治參與的要素」以及「數位工具的幫助與限制」作為研究背景。
\subsection{網路社群與資訊科技社群}
\label{sec:orgc9ad6e5}
...
\label{sec:orgfa230a6}
\subsection{設計敏捷方法問題定義與原型開發}
\label{sec:org636dd2b}
...
\item 參與途徑說明
\label{sec:org4eb262a}

本節說明 圖 \ref{fig:service-blueprint} 各階段細節。

\begin{enumerate}
\item 前期議題設定：
\label{sec:org6c4a863}
...
\item 數位原民參與手冊供網路社群理解政策形成機制與公民參與方法\footnote{主要與現為軟體工程師的樹黨發起人討論。 樹黨發起人之目的為倡議 Public Money Public Code，撰有〈二十年後的政府軟體〉一文 \url{http://treesparty.tw/2018/06/17/public-money-public-code/}。\label{org26377a8}}
\end{enumerate}

\section{線上線下整合的網路參與機制設計 }
\label{sec:orgceeb0eb}
...
\label{sec:orgf174ca5}
\begin{enumerate}
\item 應用場景
\label{sec:org14300f8}
...
\subsection{線上線下串接案例 }
\label{sec:orgbfddf2b}
前述分別介紹線上意見整理工具 sense.tw 與線下的議題小聚，此節以案例介紹如何串接線上數位工具與線下討論，加深網路社群討論科技政策之品質。
\begin{enumerate}
\item Public Money, Public Code
\label{sec:org9fdc732}

Public Money, Public Code 為國際開源社群長期推動由政府資助開發的軟體應開放原始碼的運動\footnote{Public Money, Public Code 活動由歐洲自由軟體基金會發起，要求制定法律，明訂以公務機關的經費，為公務機關開發的軟體，必須以自由開源軟體授權釋出，截至 2018/12/21 為止全球已有 158 個組織與 19130 個人已經連署公開信表達支持。詳見 \url{https://publiccode.eu/zh-tw/}。}。在開源人年會（COSCUP）中大高雄 Linux 協會（KaLUG）與樹黨\textsuperscript{\ref{org26377a8}}開始討論，於是舉辦之議題工作坊以此為主題進行討論。
...
\section{附錄}
\label{sec:org6070ddd}
\subsection{訪談對象與訪談形式}
\label{sec:org3db5d63}
{\small \rowcolors[]{2}{contiYellow!5}{contiYellow!20}
...
\end{itemize}
\item 訪談問題
\label{sec:org280938a}
\begin{enumerate}
\item 怎樣蒐集社群意見、倡議產生政策影響力
\label{sec:org55b6bc9}
簡述政策參與（促進）的經驗
\begin{enumerate}
\item 政府部門參與過的政策形式，什麼樣的方式參與？與政府開會的角色是什麼？
\label{sec:org1d0a0af}
\item 用什麼架構分析政策形成、公民參與？
\label{sec:orgf08e249}
\end{enumerate}
\item 政策參與（促進）的方式
\label{sec:org98e9388}
\begin{enumerate}
\item 怎麼倡議？
\label{sec:orgf2c3a36}
\item 怎麼組織？ 會舉辦實體會議嗎？）
\label{sec:org74a18dc}
\item 現行體制（政府意見陳達、溝通」）問題點
\label{sec:orgc96594e}
\item 案例建議與原因？
\label{sec:org6cb6f39}
\item 任何範本、方法？
\label{sec:org4e5ee94}
\end{enumerate}
\item 促進社群討論的方式
\label{sec:org9adfbba}
\item 如何保持內部成員資訊即時更新、通知同步
\label{sec:org83774a5}
\item 如何鼓勵/促進內部人員討論（討論風氣、鼓勵發言的文化）
\label{sec:org5541f4e}
\item 使用的討論工具
\label{sec:orgafe1404}
\item 使用什麼工具協助討論？
\label{sec:org97091cb}
\item 使用什麼平台作團隊內知識分享、討論？
\label{sec:orgadb84d8}
\item 平台的優缺點?
\label{sec:org6f3932c}
\item 哪些是覺得必要的功能？
\label{sec:org125e2ab}
\end{enumerate}
\end{enumerate}


\subsection{議題釐清工具相關開發資訊}
\label{sec:org98c518d}
\begin{enumerate}
\item 源碼庫
\label{sec:orgfb656a6}
\begin{enumerate}
...
\end{enumerate}
\item 部落格
\label{sec:org2b7b55c}
\url{https://medium.com/sense-tw}
\end{enumerate}
\subsection{民眾語彙腳本}
\label{sec:org142e704}
\begin{enumerate}
\item 對公部門介紹何為網路社群應如何比喻
\label{sec:orgd313f6a}
