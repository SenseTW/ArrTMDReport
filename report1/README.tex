...
\end{enumerate}
\section{研究方法 }
\label{sec:org78e1e25}
\subsection{研究架構}
\label{sec:org514f2c6}
\begin{figure}[htbp]
\centering
...
\label{sec:org9dd2e56}
\begin{enumerate}
\item 文獻檢閱
\label{sec:orga6a1440}
...
\bottomrule
\end{tabular}
\end{table}\footnotetext[4]{\label{org0a5c002}MIT 授權允許被授權人「有權利使用、複製、修改、合併、出版發行、散布、再授權和/或販售軟體及軟體的副本，及授予被供應人同等權利，惟服從以下義務：『在軟體和軟體的所有副本中都必須包含以上著作權聲明和本許可聲明。』」，中文說明 2018 年 12 月 22 日摘自維基百科\url{https://zh.wikipedia.org/wiki/MIT\%E8\%A8\%B1\%E5\%8F\%AF\%E8\%AD\%89}，英文 MIT 協定全文請見：\url{https://opensource.org/licenses/MIT}。}\footnotetext[5]{\label{orgb3ddd74}CC BY-SA（姓名標示-相同方式分享） 3.0 台灣之授權條款為依據「創用 CC 公眾授權條款」中授權姓名標示─相同方式分享。本授權條款允許使用者重製、散布、傳輸以及修改著作（包括商業性利用）。若使用者修改該著作時，僅得依本授權條款或與本授權條款類似者來散布該衍生作品。使用時必須按照著作人指定的方式表彰其姓名。詳細授權條款請見：\url{https://creativecommons.org/licenses/by-sa/3.0/tw/legalcode}}\footnotetext[6]{\label{org212ad84}CC BY-SA 4.0 (Attribution-Sharealike) International，詳細英文授權條款請見：\url{https://creativecommons.org/licenses/by-sa/4.0/}。}
\end{enumerate}

\section{文獻檢閱 }
\label{sec:org40bc8ca}

本章回顧「網路社群與資訊科技社群」、「科技治理體系及溝通缺口」、「線上公民與政治參與的要素」以及「數位工具的幫助與限制」作為研究背景。
\subsection{網路社群與資訊科技社群}
\label{sec:orgdd528e7}
...
\label{sec:org922a0d5}
\subsection{設計敏捷方法問題定義與原型開發}
\label{sec:orgeac7b72}
...
\item 參與途徑說明
\label{sec:orgcf77447}

本節說明 圖 \ref{fig:service-blueprint} 各階段細節。

\begin{enumerate}
\item 前期議題設定：
\label{sec:orgc4d5163}
...
\item 數位原民參與手冊供網路社群理解政策形成機制與公民參與方法\footnote{主要與現為軟體工程師的樹黨發起人討論。 樹黨發起人之目的為倡議 Public Money Public Code，撰有〈二十年後的政府軟體〉一文 \url{http://treesparty.tw/2018/06/17/public-money-public-code/}。\label{org9ff7e12}}
\end{enumerate}

\section{線上線下整合的網路參與機制設計 }
\label{sec:org2ea99e9}
...
\label{sec:org25f534a}
\begin{enumerate}
\item 應用場景
\label{sec:org2a2c48b}
...
\section{附錄}
\label{sec:org0d7318a}
\subsection{訪談對象與訪談形式}
\label{sec:org5cfe46d}
{\small \rowcolors[]{2}{contiYellow!5}{contiYellow!20}
...
\end{itemize}
\item 訪談問題
\label{sec:org02d285b}
\begin{enumerate}
\item 怎樣蒐集社群意見、倡議產生政策影響力
\label{sec:org8a9481d}
簡述政策參與（促進）的經驗
\begin{enumerate}
\item 政府部門參與過的政策形式，什麼樣的方式參與？與政府開會的角色是什麼？
\label{sec:org6340135}
\item 用什麼架構分析政策形成、公民參與？
\label{sec:org7ced36b}
\end{enumerate}
\item 政策參與（促進）的方式
\label{sec:orge2e38f0}
\begin{enumerate}
\item 怎麼倡議？
\label{sec:org19553aa}
\item 怎麼組織？ 會舉辦實體會議嗎？）
\label{sec:org44e8d4a}
\item 現行體制（政府意見陳達、溝通」）問題點
\label{sec:org2e7e3c7}
\item 案例建議與原因？
\label{sec:orgf836abf}
\item 任何範本、方法？
\label{sec:orga7b71e3}
\end{enumerate}
\item 促進社群討論的方式
\label{sec:org06b00e9}
\item 如何保持內部成員資訊即時更新、通知同步
\label{sec:orge676a84}
\item 如何鼓勵/促進內部人員討論（討論風氣、鼓勵發言的文化）
\label{sec:org3b10e4a}
\item 使用的討論工具
\label{sec:orge1f6253}
\item 使用什麼工具協助討論？
\label{sec:orgb41101d}
\item 使用什麼平台作團隊內知識分享、討論？
\label{sec:orgefc7db6}
\item 平台的優缺點?
\label{sec:org3eb4163}
\item 哪些是覺得必要的功能？
\label{sec:orgb34aa4b}
\end{enumerate}
\end{enumerate}


\subsection{議題釐清工具相關開發資訊}
\label{sec:org9b6ea0c}
\begin{enumerate}
\item 源碼庫
\label{sec:orgf68c494}
\begin{enumerate}
...
\end{enumerate}
\item 部落格
\label{sec:orgfc70266}
\url{https://medium.com/sense-tw}
\end{enumerate}
\subsection{民眾語彙腳本}
\label{sec:org416edda}
\begin{enumerate}
\item 對公部門介紹何為網路社群應如何比喻
\label{sec:org09e7ab9}
